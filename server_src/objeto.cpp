...
Objeto::Objeto(float x, float y, float w, float h):
        x(x), y(y), width(w), height(h), borrar(false) {}
...
void Ente::RecibirDanio(int d) { vida -= d; }
...
void Arma::disparar(std::map<Objeto*, Objeto*>& objetos, float x, float w, float y, float h,
                    int d) {
    int aux;
