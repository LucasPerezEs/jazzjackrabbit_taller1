...
EventHandler::EventHandler(ClientProtocol& protocol, bool& menu, SoundManager& sound_manager):
        protocol(protocol), was_closed(false), in_menu(menu), sound_manager(sound_manager) {}
        ...
        switch (event.type) {
            case SDL_MOUSEBUTTONDOWN:
                if (in_menu && event.button.button == SDL_BUTTON_LEFT) {
                    if (800 / 4 + 800 / 4 - 70 < event.button.x &&
                        event.button.x < 800 / 4 + 800 / 4 + 70 &&
                        600 / 4 + 600 / 4 - 15 < event.button.y &&
                        event.button.y < 600 / 4 + 600 / 4 + 15) {
