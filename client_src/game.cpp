...
/*
Hay dos formas en la que se hizo esto. La primera solo requiere el uso de los case:
SDL_MOUSEBUTTONDOWN y case: SDL_MOUSEBUTTONUP, en este caso no se ve como se arrastra cada
cuadricula hacia su posicion.

De la otra forma en la que si se ve es con lo agregado en case: SDL_MOUSEMOTION, pero este metodo
requiere de agregar fuera del switch un for que va desde 0 a numTiles. Si se saca esta funcionalidad
ese for deberia borrarse asi como tambien el this->window.fill(); que se realiza allí.
*/
void Game::create_map() {
    ...
    int numTiles = (width_texture / 16) *
                   (height_texture / 16);    // Numero total de tiles de 16x16 en el asset.
        ...
        while (SDL_PollEvent(&event)) {
            switch (event.type) {
                ...
                case SDL_MOUSEBUTTONDOWN: {
                    ...
                    // Me aseguro de que newX está por fuera de la imagen del asset y dentro de la
                    // ventana en X.
                    ...
                    // Esto es para que los bloques de las texturas se posiciones siempre en una
                    // posicion de 16x16 y no que puedas poner la mitad de un asset sobre la mitad
                    // de otro. Si no crear un mapa con esta herramienta es un quilombo.
                    ...
                    mapTiles[posicion] = selectedTile;  //({posicion, selectedTile}); //Si uso este
                                                        // ya no necesito usar tiles_map (Creo)
                ...
                case SDL_MOUSEMOTION: {
                    if (mouseHeldDown) {
                        ...
                        // Me aseguro de que newX está por fuera de la imagen del asset y dentro de
                        // la ventana en X.
                        double minX = width_texture;
                        double maxX = window_width;
                        if (newX < minX || newX > maxX)
                        ...
                        // Esto es para que los bloques de las texturas se posiciones siempre en una
                        // posicion de 16x16 y no que puedas poner la mitad de un asset sobre la
                        // mitad de otro. Si no crear un mapa con esta herramienta es un quilombo.
                        int multiploX = std::floor(newX / 16) * 16;
                        int multiploY = std::floor(newY / 16) * 16;
                        ...
                        mapTiles.insert({posicion, selectedTile});  // Si uso este ya no necesito
                                                                    // usar tiles_map (Creo)
                    }
                    break;
                }
            ...
            // Esto muestra los assets colocados durante la creacion del mapa (es el mapa creado
            // visualmente)
            for (size_t i = 0; i < tiles_mapa.size(); i++)
                SDL_RenderCopy(renderer, assetTexture, &(tiles_mapa[i].srcRect),
                               &(tiles_mapa[i].destRect));
